...
const RETRY_DELAYS = [10000, 20000, 30000]; // exponential backoff
...
async function getAdCreatives(adIds, token) {
  ...
  for (let i = 0; i < adIds.length; i += 50) {
    ...
    for (const adId of batch) {
      try {
        const fields = 'creative{id,thumbnail_url,image_url,video_id,object_story_id,title,body,call_to_action_type}';
        const url = `${BASE_URL}/${adId}?fields=${encodeURIComponent(fields)}&access_token=${token}`;
        const data = await requestWithRetry(url);
        results.push({ ad_id: adId, creative: data.creative || {} });
      } catch(e) {
        // Skip individual creative fetch failures
