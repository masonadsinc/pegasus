...
class MetaSync {
  constructor(dbConfig, metaToken, orgId, supabaseClient) {
  ...
  async syncStructure(account) {
    ...
    // Store new creative images to Supabase Storage (permanent URLs)
    if (this.supabase) {
      await this.storeCreativeImages(dbAccountId);
    }
  }
  ...
  /**
   * Store creative images in Supabase Storage for permanent URLs.
   * Only processes ads that don't have a stored_creative_url yet.
   */
  async storeCreativeImages(dbAccountId) {
    const res = await this.db.query(
      "SELECT id, platform_ad_id, ad_account_id, creative_url FROM ads WHERE ad_account_id = $1 AND creative_url IS NOT NULL AND stored_creative_url IS NULL",
      [dbAccountId]
    );
    if (res.rows.length === 0) return;

    console.log(`    📦 Storing ${res.rows.length} new creative images to Supabase Storage...`);
    let stored = 0;

    for (const ad of res.rows) {
      try {
        const imgRes = await fetch(ad.creative_url, { signal: AbortSignal.timeout(15000) });
        ...
        const path = `${ad.ad_account_id}/${ad.platform_ad_id}.${ext}`;

        const { error } = await this.supabase.storage.from('ad-creatives').upload(path, buffer, { contentType: ct, upsert: true });
        if (error) { console.log(`    ⚠️ Storage upload error for ${ad.platform_ad_id}: ${error.message}`); continue; }

        const { data: urlData } = this.supabase.storage.from('ad-creatives').getPublicUrl(path);
        const publicUrl = urlData?.publicUrl;
        if (!publicUrl) continue;

        await this.db.query('UPDATE ads SET stored_creative_url = $1, stored_thumbnail_url = $1 WHERE id = $2', [publicUrl, ad.id]);
        stored++;
      } catch (e) {
    ...
    if (stored > 0) console.log(`    📦 Stored ${stored}/${res.rows.length} images`);
  }
  ...
  async syncAccount(account, dateStart, dateEnd, options = {}) {
    ...
    try {
      ...
      if (!account._retried) {
        console.log(`  🔄 Retrying ${account.name} in 30s...`);
        await new Promise(r => setTimeout(r, 30000));
        account._retried = true;
        try {
          await this.syncAccount(account, dateStart, dateEnd, options);
          console.log(`  ✅ Retry succeeded for ${account.name}`);
          return;
        } catch(e2) {
          console.log(`  ❌ Retry also failed for ${account.name}: ${e2.message}`);
        }
      }
      this.stats.errors++;
