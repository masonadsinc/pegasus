...
class MetaSync {
  ...
  async getAccounts() {
    const res = await this.db.query(
      'SELECT id, platform_account_id, name, objective, primary_action_type, target_cpl, target_roas FROM ad_accounts WHERE org_id = $1 AND is_active = true',
      [this.orgId]
    );
    return res.rows;
  }
  ...
  async syncInsights(account, level, dateStart, dateEnd) {
    ...
    const rows = await api.getInsights(accountId, this.token, apiLevel, dateStart, dateEnd);
    ...
    for (const r of rows) {
      ...
      if (primary_action_type) {
        ...
        const value = actionValues[primary_action_type] || 0;
        ...
        schedules = actions.schedule_total || 0;
      }
  ...
  async syncAccount(account, dateStart, dateEnd, options = {}) {
    const { skipBreakdowns = false, levels = ['campaign', 'ad_set', 'ad'] } = options;
    ...
    try {
      ...
      // Breakdowns (ad level only by default to save API calls)
      if (!skipBreakdowns) {
        await this.syncBreakdowns(account, 'ad', dateStart, dateEnd);
      }
  ...
  async syncAll(dateStart, dateEnd, options = {}) {
    ...
    // Log the sync
    await this.db.query(`
      INSERT INTO sync_logs (org_id, sync_type, level, date_range_start, date_range_end, records_synced, errors, duration_ms, completed_at, status)
      VALUES ($1, $2, 'all', $3, $4, $5, $6, $7, now(), $8)
    `, [
      this.orgId,
