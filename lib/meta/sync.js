...
class MetaSync {
  ...
  async syncStructure(account) {
    ...
    // Refresh all creative URLs (Meta CDN URLs expire daily)
    await this.refreshCreativeUrls(dbAccountId);
  }
  ...
  /**
   * Refresh creative URLs for ALL ads (Meta CDN URLs expire after ~24h)
   */
  async refreshCreativeUrls(dbAccountId) {
    // Only refresh active/paused ads (skip deleted — they won't have valid URLs anyway)
    const res = await this.db.query(
      "SELECT platform_ad_id FROM ads WHERE ad_account_id = $1 AND creative_url IS NOT NULL AND status IN ('ACTIVE', 'PAUSED')",
      [dbAccountId]
    );
    const adIds = res.rows.map(r => r.platform_ad_id);
    if (adIds.length === 0) return;

    console.log(`    🔄 Refreshing creative URLs for ${adIds.length} ads...`);
    ...
    for (let i = 0; i < adIds.length; i += 50) {
      ...
      try {
        const creatives = await api.getAdCreatives(batch, this.token);
        for (const c of creatives) {
          const cr = c.creative;
          if (!cr) continue;
        ...
        console.log(`    ⚠️ Creative refresh batch error: ${e.message}`);
      }
    }

    console.log(`    🔄 Refreshed ${refreshed}/${adIds.length} creative URLs`);
  }
